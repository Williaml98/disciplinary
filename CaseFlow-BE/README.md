...
- **Registration email verification**: self-registration is a real three-step flow, not client-side theater. `POST /api/auth/register/otp` generates a 6-digit code server-side, holds it in `OtpService` (in-memory, keyed by `purpose+email` — 10-minute expiry, 60-second resend cooldown, 5 incorrect-attempt cap), and emails it via `EmailService`. `POST /api/auth/register/otp/verify` checks a code without consuming it (gives the FE immediate feedback on step 2). `POST /api/auth/register` itself re-verifies and **consumes** the code as the actual security boundary — the code is never sent back to the client, only ever to the registrant's inbox.
- **Forgot password**: `POST /api/auth/password/reset/otp` emails a reset code to an existing account (404 if the email isn't registered), reusing the same `OtpService` under a distinct `reset-password` purpose so it can never be satisfied with a stray registration code for the same address. `POST /api/auth/password/reset` verifies and consumes the code, updates the password, and — like `/register` — logs the caller straight in by returning `{ token, user }`.
...
| `POST /api/auth/password/reset/otp` | Open | `{ email }` → emails a reset code (404 if no account, 429 if resent too soon) |
| `POST /api/auth/password/reset` | Open | `{ email, otp, newPassword }` → verifies and consumes the code, updates the password → `{ token, user }` |
